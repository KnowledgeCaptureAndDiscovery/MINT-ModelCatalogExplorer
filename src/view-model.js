                ...
                <div>
                  <template is="dom-if" if="[[_checkValue(item.input_files.value)]]">
                    ...
                    <template is="dom-repeat" items="{{item.input}}" as="stuff">
                      ...
                      <a href="[[routePath]]variable-presentation"><vaadin-button class="pointer" variable\$="{{stuff.var}}" on-click="openConfigForUri" raised="">[[stuff.file]]</vaadin-button></a>
                    [[stuff.des]]<br>
                    </template>
                  </template>
   ...
   console.log(inpV, this.finConfigs);
    ...
    if(inpV != ''){
      for(var i=0; i<this.finConfigs.results.bindings.length; i++){
       // console.log("hi")
        if(this.finConfigs.results.bindings[i].version.value[0] === inpV){
